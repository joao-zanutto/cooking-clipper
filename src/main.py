...
def process_video(video_path: str, output_path: str):
    ...
    Returns ``(success, fps, scores)``.  *scores* is a numpy array of
    per-frame motion percentages; both are ``None`` on failure.
    """
    t0 = time.time()
...
def _run_local(video_path: str, output_path: str | None = None) -> int:
    ...
    t_start = time.time()
    ok, _fps, _scores = process_video(video_path, output_path)
...
def _run_s3(s3_url: str) -> int:
    ...
    scores_s3_url = build_s3_url(bucket, scores_key_for(key))
    config_s3_url = build_s3_url(bucket, config_key_for(key))

    print(f"Pipeline: {s3_url}")
    print(f"  Split:   {output_s3_url}")
    print(f"  Scores:  {scores_s3_url}")
    print(f"  Config:  {config_s3_url}")
    print("-" * 60)

    storage = S3Storage()
    t_start = time.time()

    # Temporary local paths — output always .mp4 for universal playback
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args:
        print(
            ...
            "  python -m src <local-video.mp4> [output.mp4]\n"
            "  python -m src s3://bucket/path/video.mp4\n"
            "  python -m src serve s3://bucket/path/video.mp4   # launch viewer\n"
        )
        return 1
